...
**L-009 — visibilidade decidida pelo CSS não pode ser anunciada pelo estado do React** · acessibilidade, responsividade
Ao transformar a sidebar em gaveta (F5-2), o caminho automático foi deslocá-la para fora da tela quando fechada (`-translate-x-full`) e marcá-la com `aria-hidden={!isOpen}`. Os dois estão errados, e por motivos diferentes. O deslocamento deixa a navegação **invisível e ainda focável**: o Tab percorre cinco links que ninguém vê. E o `aria-hidden` vindo do estado não tem como estar certo, porque **quem decide se a gaveta existe é o `lg:` do CSS, não o React** — em tela larga a sidebar está visível com `isOpen === false`, e o atributo a esconderia de quem usa leitor de tela justamente onde ela está na tela.
→ Deixe a visibilidade inteira nas mãos do CSS, com um par de classes que resolve os dois casos: fechada, `hidden lg:flex` (fora do fluxo em tela estreita, coluna em tela larga); aberta, `fixed … flex lg:static`. `display: none` já remove da ordem de tabulação e da árvore de acessibilidade, sem atributo nenhum. O estado do React fica só com o que é mesmo dele: o `aria-expanded` do botão que abre. De brinde, o teste continua enxergando os links — o jsdom não aplica a folha do Tailwind, então a asserção de navegação não precisa saber que existe uma gaveta.

**L-008 — o mock que calcula vira um segundo dono da regra** · arquitetura, testes
Ao integrar (F5-1), a decisão de deixar os indicadores com o backend resolveu a divergência entre repositórios — e quase a reintroduziu pela porta dos fundos. A camada mock precisava devolver `indicadores` para as telas funcionarem sem API, e o caminho automático era calcular ali. Seria a mesma regra implementada duas vezes, com a segunda cópia escondida em `services/mock/`, que é o último lugar onde alguém procura quando o número da tela não bate com o da API.
→ Fixture serve **resposta gravada**, não dado gerado: os indicadores vêm capturados junto e não há regra a manter. E serve **DTO**, não domínio, atravessando os mesmos mapeadores da resposta real — assim a fixture não tem caminho de conversão próprio e não pode divergir do contrato sem quebrar o build. Onde a escrita em modo fixture obrigou a uma aproximação (cadastrar precisa devolver algo), ela está rotulada como aproximação e deixa de fora a regra mais sujeita a divergir. Formalizado no ADR-0008.

**L-007 — contrato de API diverge em responsabilidade, não em casing** · serviços, integração
O ADR-0002 previu que a integração traria surpresa de **casing** (`budget_spent` vs `budgetSpent`) e montou a defesa para isso: um mapeador por recurso. Quando o contrato real chegou, o casing foi a parte fácil — a divergência que custou foi de **responsabilidade**: a API resolve as relações na resposta (`client: {id,name}` em vez de `client_id`), calcula os indicadores e tem duas formas de projeto (lista sem `objective`, detalhe com). Isso não muda um mapeador: muda o tipo de domínio, os hooks, as telas e o que `domain/` ainda tem o direito de calcular.
→ Ao planejar integração, a pergunta cara não é "como os campos se chamam", e sim **"quem decide o quê"**. Leia a seção de tipos e a de armadilhas do contrato antes de estimar. O mapeador por recurso continuou valendo a pena — sem ele, as três mudanças de forma teriam vazado para todas as telas de uma vez.

**L-006 — formulário de edição sem `setState` dentro de efeito** · hooks, formulários
F2-4 precisa abrir o formulário já preenchido com o projeto carregado. O caminho automático — `useEffect(() => setValues(toFormValues(project)), [project])` — é exatamente o que a L-004 proíbe, e a saída não é óbvia: o valor inicial só existe **depois** da carga, então "usar como estado inicial" parece impossível.
→ Não monte o editor enquanto não houver dado. A página resolve carregando/erro/pronto e só no estado pronto renderiza `<ProjectEditor project={…} key={project?.id ?? 'new'} />`; lá dentro, `useState(toFormValues(project))` é estado inicial de verdade, porque o componente nasce com o projeto em mãos. A `key` cobre a troca de projeto sem sair da rota. O mesmo formato serve a qualquer tela de edição — F5 em diante, comece por aí.

**L-005 — `userEvent.type` não preenche `<input type="date">` no jsdom** · testes, ferramental
Os testes do formulário de projeto (F2-3) falhavam com o campo de data vazio, mesmo com o `type` aparentemente correto. O jsdom aplica a higienização de valor do HTML: em `type="date"`, todo valor que não seja uma data completa e válida vira `""`. Como `userEvent.type` digita caractere por caractere, cada estado intermediário (`2`, `20`, `202`…) é inválido e o campo é zerado — o valor final nunca chega. O sintoma engana porque o erro que aparece é "Informe a data de início", a mensagem de validação, e a suspeita cai na regra e não na digitação.
→ Em campo de data (e em qualquer input com higienização de valor), use `fireEvent.change(input, { target: { value: '2026-02-01' } })`, que aplica o valor de uma vez. Para texto, número e select, `userEvent` continua sendo o certo. O helper `setField` em `ProjectFormPage.test.tsx` é o modelo.
